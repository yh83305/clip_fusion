...
def integrate(
    ...
    valid_pts = (depth_val > 0.) & (
            depth_diff >= -sdf_trunc)  # all points 1. inside frustum 2. with valid depth 3. outside -truncate_dist
    ...
    if color_vol is not None and color_im is not None:
        ...
        color_vol[valid_vox_x, valid_vox_y, valid_vox_z, :] = (w_old[:,
                                                               None] * old_color + obs_weight * new_color) / w_new[:,
                                                                                                             None]

    return weight_vol, tsdf_vol, color_vol, pix_x, pix_y, valid_pix, valid_pts, valid_vox_x, valid_vox_y, valid_vox_z


def feature_integrate(
...
class TSDFVolumeTorch:
    ...
    def __init__(self, voxel_dim, origin, voxel_size, device, margin=3, fuse_color=False, fuse_feature=False):
        """
        Args:
        ...
        self.feature_integrate_func = feature_integrate
        ...
        self.feature_vol = None
        self.feature_weight_vol = None
    ...
    def reset(self):
        ...
        if self.fuse_feature:
            self.feature_vol = torch.zeros(*self.vol_dim, 512).to(self.device)
            self.feature_weight_vol = torch.zeros(*self.vol_dim, 1).to(self.device)
    ...
    def integrate(self, depth_im, cam_intr, cam_pose, obs_weight, color_img=None):
        ...
        if color_img is not None:
            detect_img = color_img
            color_img = self.data_transfer(color_img)
        else:
            color_img = None
        im_h, im_w = depth_im.shape
        # fuse
        weight_vol, tsdf_vol, color_vol, pix_x, pix_y, valid_pix, valid_pts, valid_vox_x, valid_vox_y, valid_vox_z = self.integrate_func(
            depth_im,
            cam_intr,
            cam_pose,
            obs_weight,
        ...
        if self.fuse_feature and detect_img is not None:
            if self.comparator.judge_similarity(detect_img, 30):
                # clip_val(N) > valid_pix(n) > valid_pts(m) > tsdf_indices(k) > valid_features(t)
                self.detector.detect(detect_img)
                # print(self.feature_vol.shape)  # (W,H,D,512)
                clip_val = torch.zeros((pix_x.shape[0], 512)).to(self.device)  # (N,512)
                # print("pix_x.shape(N)", pix_x.shape)
                # print("valid_pts.shape(n)", valid_pts.shape)

                tsdf_indices = torch.logical_and(self.tsdf_vol[valid_vox_x, valid_vox_y, valid_vox_z] >= -0.2,
                                                 self.tsdf_vol[valid_vox_x, valid_vox_y, valid_vox_z] <= 0.2)  # (m)
                # print("tsdf_indices.shape(m)", tsdf_indices.shape)

                tsdf_pix_x = pix_x[valid_pix][valid_pts][tsdf_indices]  # (k,512)
                tsdf_pix_y = pix_y[valid_pix][valid_pts][tsdf_indices]  # (k,512)
                # print("tsdf_pix_x.shape(k)", tsdf_pix_x.shape)

                change1 = torch.zeros_like(clip_val[valid_pix][valid_pts]).to(self.device)  # (m,512)
                change2 = torch.zeros_like(clip_val[valid_pix]).to(self.device)  # (n,512)
                change1[tsdf_indices] = self.data_transfer(
                    self.detector.query_mask_vector(tsdf_pix_x, tsdf_pix_y))  # (k,512)
                change2[valid_pts] = change1
                clip_val[valid_pix] = change2
                features_valid_pts = torch.all(clip_val != 0, axis=1)  # (N)
                # print("features_valid_pts.shape(m)", features_valid_pts.shape)
                feature_valid_vox_x = self.vox_coords[features_valid_pts, 0]  # (t)
                feature_valid_vox_y = self.vox_coords[features_valid_pts, 1]  # (t)
                feature_valid_vox_z = self.vox_coords[features_valid_pts, 2]  # (t)

                # print("feature_valid_vox_x.shape(t)", feature_valid_vox_x.shape)

                valid_features = clip_val[features_valid_pts]  # (t,512)
                print("valid_features.shape(t)", valid_features.shape[0])

                # 从全体点列表取出
                features_w_old = self.feature_weight_vol[
                    feature_valid_vox_x, feature_valid_vox_y, feature_valid_vox_z]  # (t)
                features_vals = self.feature_vol[
                    feature_valid_vox_x, feature_valid_vox_y, feature_valid_vox_z]  # (t,512)

                features_w_new = features_w_old + obs_weight

                # 注入全体列表
                self.feature_vol[
                    feature_valid_vox_x, feature_valid_vox_y, feature_valid_vox_z] = (
                                                                                             features_w_old * features_vals + obs_weight * valid_features) / features_w_new  # (t,512)
                self.feature_weight_vol[
                    feature_valid_vox_x, feature_valid_vox_y, feature_valid_vox_z] = features_w_new  # (t)
    ...
    def save_features(self, name):
        torch.save(self.feature_vol, name)
    ...
    def render_model(self, c2w, intri, imh, imw, near=0.5, far=5., n_samples=192):
        ...
        cost_matrix = sign_matrix * torch.arange(n_samples, 0, -1).float().to(self.device)[None, None,
                                    :]  # [h, w, n_samples]
    ...
    def render_pyramid(self, c2w, intri, imh, imw, n_pyr=4, near=0.5, far=5., n_samples=192):
        ...
        for l in range(n_pyr):
            dep, rgb, feat, vtx, nrm, mask = self.render_model(c2w, K, imh, imw, near=near, far=far,
                                                               n_samples=n_samples)
            dep_pyr += [dep]
            rgb_pyr += [rgb]
            vtx_pyr += [vtx]
