...
def warp_features(Feat, u, v, mode='bilinear'):
    ...
    Feat_warped = F.grid_sample(Feat.unsqueeze(0).permute(0, 3, 1, 2), uv_grid, mode=mode, padding_mode='border',
...
def compute_vertex(depth, K):
    ...
    vertex = torch.stack([(i - cx) / fx, (j - cy) / fy, torch.ones_like(i)], -1).to(device) * depth[
        ..., None]  # [h, w, 3]
    return vertex
...
def invH(H, damp=1e-3):
    ...
    H_damp = H + damp * torch.eye(H.size(0)).to(H.device)

    # Compute inverse of damped Hessian matrix
    try:
        if H.is_cuda:
            invH = torch.inverse(H_damp.cpu()).cuda()
        else:
            invH = torch.inverse(H_damp)
        return invH
