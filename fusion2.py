...
def integrate(
    ...
    pix_valid_pts_x = pix_x[valid_pix][valid_pts]
    pix_valid_pts_y = pix_y[valid_pix][valid_pts]
    ...
    return weight_vol, tsdf_vol, color_vol, pix_valid_pts_x, pix_valid_pts_y, valid_vox_x, valid_vox_y, valid_vox_z
...
class TSDFVolumeTorch:
    ...
    def integrate(self, depth_im, cam_intr, cam_pose, obs_weight, color_img=None):
        ...
        # fuse
        weight_vol, tsdf_vol, color_vol, pix_valid_pts_x, pix_valid_pts_y, valid_vox_x, valid_vox_y, valid_vox_z = self.integrate_func(
            depth_im,
            cam_intr,
            cam_pose,
            obs_weight,
        ...
        if self.fuse_feature and detect_img is not None:
            if self.comparator.judge_similarity(detect_img, 20):
                # clip_val(N) > valid_pix(n) > valid_pts(m) > tsdf_indices(k) > valid_features(t)
                if self.detector.detect(detect_img):
                    # print(self.feature_vol.shape)  # (W,H,D,512)
                    clip_val = torch.zeros((pix_valid_pts_x.shape[0], 512)).to(self.device)  # (m,512)
                    # print("pix_valid_pts_x.shape(m)", pix_valid_pts_x.shape)
                    ...
                    tsdf_pix_x = pix_valid_pts_x[tsdf_indices]  # (k,512)
                    tsdf_pix_y = pix_valid_pts_y[tsdf_indices]  # (k,512)
                    # print("tsdf_pix_x.shape(k)", tsdf_pix_x.shape)

                    clip_val[tsdf_indices] = self.data_transfer(
                        self.detector.query_mask_vector(tsdf_pix_x, tsdf_pix_y))  # (k,512)
                    features_valid_pts = torch.all(clip_val != 0, axis=1)  # (m)
                    # print("features_valid_pts.shape(m)", features_valid_pts.shape)
                    feature_valid_vox_x = valid_vox_x[features_valid_pts]  # (t)
                    feature_valid_vox_y = valid_vox_y[features_valid_pts]  # (t)
                    feature_valid_vox_z = valid_vox_z[features_valid_pts]  # (t)
    ...
    @torch.no_grad()
    def render_model(self, c2w, intri, imh, imw, near=0.1, far=6., n_samples=192):
        """
